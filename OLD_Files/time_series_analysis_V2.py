...
def plot_cooperation_proportion(df, column_prefix, stage_count):
    ...
    for idx in range(len(df)):
        ...
        ax1.scatter(range(1, stage_count + 1), cooperation_proportions,  marker='o', label=f'{skill} Row {idx + 1}', color=color, edgecolors='black',linewidth=1)
   
    ax1.set_xlabel('Iteration')
    ax1.set_ylabel('Cooperation Proportion')
    ax1.set_title('Cooperation Proportion Across Stages')
    ...
    for idx in range(len(df)):
        ...
        cooperation_proportions = [df.at[idx, f'{column_prefix}_stage_{i+1}'] for i in range(stage_count)]
        ax2.scatter(range(1, stage_count + 1), cooperation_proportions,  marker=marker_shape, label=f'{skill} Row {idx + 1}', color='white', edgecolors='black',linewidth=1)
           
    ax2.set_xlabel('Iteration')
    ax2.set_ylabel('Cooperation Proportion')
    ax2.set_title('Cooperation Proportion Across Stages')
    # Legend for marker shapes indicating group sizes
    shape_handles = [
        plt.Line2D([0], [0], color='white', marker='o', linestyle='None', markersize=10, markeredgecolor='black', label='<= 100 agents'),
        plt.Line2D([0], [0], color='black', marker='o', linestyle='None', markersize=10, markeredgecolor='black', label='> 100 agents')
      ]
